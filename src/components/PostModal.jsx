...
export const PostModal = ({ setpostData, postData }) => {
  ...
  const handleNewsLike = async (item) => {
    ...
    setNewsLikes((prev) => ({
      ...
      [item.id]: Math.max(
        0,
        (prev[item.id] || 0) + (isCurrentlyLiked ? -1 : 1)
      ),
    }));
      ...
      if (!postDoc.exists()) {
        ...
        if (isCurrentlyLiked) {
          ...
          await updateDoc(globalPostRef, {
            likes: increment(-1),
            likedBy: currentData.likedBy.filter(
    ...
    try {
      ...
      if (recipientFcmToken) {
        await sendNotification(
          recipientFcmToken,
          `${user.displayName}: has commented on your Post`,
          user.uid,
          user.displayName,
          user.photoURL
        );
          ...
          <div className="w-full md:w-3/5 h-[40vh] md:h-auto relative bg-black">
            ...
            {isHeartAnimating && (
              <div className="absolute inset-0 flex items-center justify-center">
                <HeartFilled className="text-4xl md:text-6xl text-red-500 animate-like-heart" />
              </div>
            )}
              ...
              <div className="flex justify-between items-center">
                <div className="flex space-x-3 md:space-x-4">
                  <Button
                    ...
                    onClick={() =>
                      selectedPost.isNews
                        ? handleNewsLike(selectedPost)
                        : handleLike(selectedPost)
                    }
                  >
                    <span className="ml-1">
                      {selectedPost.isNews
                        ? newsLikes[selectedPost.id] || 0
                        : selectedPost.likes}
                    </span>
                  </Button>
                  <Button
          ...
          <div className="w-full md:w-2/5 flex flex-col bg-white h-[50vh] md:h-auto ">
            ...
            <div className="p-3 md:p-4 border-b flex-shrink-0 max-h-[30vh] overflow-y-auto">
              <div className="flex items-center space-x-3 mb-2">
                ...
                />
                <div>
                  <span className="font-semibold block text-violet-900">
                    {user.displayName}
                  </span>
    ...
    {selectedPost.isNews ? (
      newsComments[selectedPost.id]?.length > 0 ? (
        newsComments[selectedPost.id].map((comment, index) => (
          <div key={index} className="p-2 md:p-4 border-b">
            <div className="flex items-start space-x-2 md:space-x-3">
              <Avatar src={comment.userPhoto} size={32} className="flex-shrink-0" />
        ...
        />
        <Button
          className="bg-violet-600 hover:bg-violet-500 text-black text-sm md:text-base h-8 md:h-auto"
